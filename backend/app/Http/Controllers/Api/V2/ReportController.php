...
class ReportController extends Controller
{
    ...
    private function report($type,$title,$stats,$columns,$rows,Request $request): array { return ['report_type'=>$type,'title'=>$title,'report_id'=>'LGU-TUAO-'.now()->format('YmdHis'),'generated_at'=>now()->toIso8601String(),'stats'=>$stats,'columns'=>$columns,'rows'=>$rows,'filters'=>['contractors'=>Contractor::where('is_active',true)->orderBy('company_name')->get(['id','company_name'])]]; }
